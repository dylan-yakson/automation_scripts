#!/bin/bash

function git_secure_checkout() {
	if [ "$CHANGE_DIR" != "" ] ; then
		cd "$CHANGE_DIR"
	fi
	path="$1"
	commit="$2"
	cd "$path"
	output="$(git checkout "${commit}^{commit}" 2>&1)"
	if [ $? -ne 0 ] ; then
		echo "[!] Commit hash $commit is absent from submodules $path !"
		echo "$output"
		cleanup
		exit 4
	fi
}

function git_secure_clone() {
	cd /tmp/kp-hardened-nginx
	repo="$1"
	commit="$2"
	folder="$(echo "$repo" | sed -E "s@https://github.com/.*/(.*)\.git@\1@")"
	output="$(git clone "$repo" 2>&1)"
	if [ $? -ne 0 ] ; then
		echo "[!] Error cloning $1"
		echo "$output"
		cleanup
		exit 2
	fi
	cd "$folder"
	output="$(git checkout "${commit}^{commit}" 2>&1)"
	if [ $? -ne 0 ] ; then
		echo "[!] Commit hash $commit is absent from repository $repo"
		echo "$output"
		cleanup
		exit 3
	fi
}

function secure_download() {
	cd /tmp/kp-hardened-nginx
	link="$1"
	file="$2"
	hash="$3"
	output="$(wget -q -O "$file" "$link" 2>&1)"
	if [ $? -ne 0 ] ; then
		echo "[!] Error downloading $link"
		echo "$output"
		cleanup
		exit 5
	fi
	check="$(sha512sum "$file" | cut -d ' ' -f 1)"
	if [ "$check" != "$hash" ] ; then
		echo "[!] Wrong hash from file $link (expected $hash got $check)"
		cleanup
		exit 6
	fi
}

function do_and_check_cmd() {
	if [ "$CHANGE_DIR" != "" ] ; then
		cd "$CHANGE_DIR"
	fi
	output=$("$@" 2>&1)
	ret="$?"
	if [ $ret -ne 0 ] ; then
		echo "[!] Error from command : $*"
		echo "$output"
		cleanup
		exit $ret
	fi
	#echo $output
	return 0
}

function cleanup() {
	echo "[*] Cleaning /tmp/kp-hardened-nginx"
	rm -rf /tmp/kp-hardened-nginx
}

function get_sign_repo_key() {
	key="-----BEGIN PGP PUBLIC KEY BLOCK-----
    Version: GnuPG v2.0.22 (GNU/Linux)

    
    =EWWI
    -----END PGP PUBLIC KEY BLOCK-----"
        echo "$key"
    }

    function get_sign_repo_key_rsa() {
        key="-----BEGIN PUBLIC KEY-----
    
    bQIDAQAB
    -----END PUBLIC KEY-----"
        echo "$key"
    }

    function get_sign_source_keys() {
        keys="-----BEGIN PGP PUBLIC KEY BLOCK-----
    Version: GnuPG v1.4.11 (FreeBSD)

    
    =Tavt
    -----END PGP PUBLIC KEY BLOCK-----
    -----BEGIN PGP PUBLIC KEY BLOCK-----

    
    =/pFc
    -----END PGP PUBLIC KEY BLOCK-----
    -----BEGIN PGP PUBLIC KEY BLOCK-----
    Version: GnuPG v1.4.11 (FreeBSD)

    
    =iFlP
    -----END PGP PUBLIC KEY BLOCK-----"
	echo "$keys"
}

# Variables
NTASK=$(nproc)

# Check if we are root
if [ $(id -u) -ne 0 ] ; then
	echo "[!] Run me as root"
	exit 1
fi

# Detect OS
OS="ubuntu"

old_dir="${PWD}"

# Create /tmp/kp-hardened-nginx
echo "[*] Prepare /tmp/kp-hardened-nginx"
if [ -e "/tmp/kp-hardened-nginx" ] ; then
	do_and_check_cmd rm -rf /tmp/kp-hardened-nginx
fi
do_and_check_cmd mkdir /tmp/kp-hardened-nginx

# Create /opt/kp-hardened-nginx
echo "[*] Prepare /opt/kp-hardened-nginx"
if [ -d "/opt/kp-hardened-nginx" ] ; then
	echo "[!] Looks like kp-hardened-nginx is already installed"
	exit 1
fi
do_and_check_cmd mkdir -p /opt/kp-hardened-nginx/deps

# Check nginx version
NGINX_VERSION="$(nginx -V 2>&1 | sed -rn 's~^nginx version: nginx/(.*)$~\1~p')"
echo "[*] Detected nginx version ${NGINX_VERSION}"
# if [ "$NGINX_VERSION" != "1.20.1" ] ; then
# 	echo "/!\\ Warning : we recommend you to use nginx v1.20.1, you should uninstall your nginx version and run this script again ! /!\\"
# fi

# Stop nginx on Linux
echo "[*] Stop nginx service"
systemctl status nginx > /dev/null 2>&1
if [ $? -eq 0 ] ; then
    do_and_check_cmd systemctl stop nginx
fi

# Install dependencies
echo "[*] Update packet list"
do_and_check_cmd apt update

echo "[*] Install compilation dependencies"
DEBIAN_DEPS="git autoconf pkg-config libpcre++-dev automake libtool g++ make libgd-dev libssl-dev wget libbrotli-dev gnupg patch libreadline-dev"
DEBIAN_FRONTEND=noninteractive do_and_check_cmd apt install -y $DEBIAN_DEPS

# Download, compile and install lua
echo "[*] Download lua-5.1.5"
secure_download "https://www.lua.org/ftp/lua-5.1.5.tar.gz" "lua-5.1.5.tar.gz" "0142fefcbd13afcd9b201403592aa60620011cc8e8559d4d2db2f92739d18186860989f48caa45830ff4f99bfc7483287fd3ff3a16d4dec928e2767ce4d542a9"
CHANGE_DIR="/tmp/kp-hardened-nginx" do_and_check_cmd tar -xzf lua-5.1.5.tar.gz
# Patch first Makefile
patch1='@@ -43,7 +43,7 @@
 # What to install.
 TO_BIN= lua luac
 TO_INC= lua.h luaconf.h lualib.h lauxlib.h ../etc/lua.hpp
-TO_LIB= liblua.a
+TO_LIB= liblua.a liblua.so
 TO_MAN= lua.1 luac.1

 # Lua version and release.'
echo "$patch1" > /tmp/kp-hardened-nginx/lua-patch-1
do_and_check_cmd patch /tmp/kp-hardened-nginx/lua-5.1.5/Makefile /tmp/kp-hardened-nginx/lua-patch-1
# Patch second Makefile
patch2='@@ -8,7 +8,7 @@
 PLAT= none

 CC= gcc
-CFLAGS= -O2 -Wall $(MYCFLAGS)
+CFLAGS= -O2 -Wall $(MYCFLAGS) -fPIC
 AR= ar rcu
 RANLIB= ranlib
 RM= rm -f
@@ -23,6 +23,7 @@
 PLATS= aix ansi bsd freebsd generic linux macosx mingw posix solaris

 LUA_A=	liblua.a
+LUA_SO= liblua.so
 CORE_O=	lapi.o lcode.o ldebug.o ldo.o ldump.o lfunc.o lgc.o llex.o lmem.o \
        lobject.o lopcodes.o lparser.o lstate.o lstring.o ltable.o ltm.o  \
        lundump.o lvm.o lzio.o
@@ -36,7 +37,7 @@
 LUAC_O= luac.o print.o

 ALL_O= $(CORE_O) $(LIB_O) $(LUA_O) $(LUAC_O)
-ALL_T= $(LUA_A) $(LUA_T) $(LUAC_T)
+ALL_T= $(LUA_A) $(LUA_T) $(LUAC_T) $(LUA_SO)
 ALL_A= $(LUA_A)

 default: $(PLAT)
@@ -54,6 +55,9 @@
 $(LUA_T): $(LUA_O) $(LUA_A)
 	$(CC) -o $@ $(MYLDFLAGS) $(LUA_O) $(LUA_A) $(LIBS)

+$(LUA_SO): $(CORE_O) $(LIB_O)
+	$(CC) -o $@ -shared -fPIC $? -ldl -lm
+
 $(LUAC_T): $(LUAC_O) $(LUA_A)
 	$(CC) -o $@ $(MYLDFLAGS) $(LUAC_O) $(LUA_A) $(LIBS)'
echo "$patch2" > /tmp/kp-hardened-nginx/lua-patch-2
do_and_check_cmd patch /tmp/kp-hardened-nginx/lua-5.1.5/src/Makefile /tmp/kp-hardened-nginx/lua-patch-2
echo "[*] Compile and install lua-5.1.5"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-5.1.5" do_and_check_cmd make -j $NTASK linux
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-5.1.5" do_and_check_cmd make INSTALL_TOP=/opt/kp-hardened-nginx/deps install

# Download, compile and install libmaxminddb
echo "[*] Download maxmind/libmaxminddb"
secure_download "https://github.com/maxmind/libmaxminddb/releases/download/1.6.0/libmaxminddb-1.6.0.tar.gz" "libmaxminddb-1.6.0.tar.gz" "9394e8dd959982d4ef5d15a928d32700722ed9d6c9988d9cc1bf2f4e67de0a53cc6987e90aaef3a6926c9ff36ac378f7a1fe47818fda4f5a3a22539210b2d004"
CHANGE_DIR="/tmp/kp-hardened-nginx" do_and_check_cmd tar -xzf libmaxminddb-1.6.0.tar.gz
echo "[*] Compile and install libmaxminddb"
CHANGE_DIR="/tmp/kp-hardened-nginx/libmaxminddb-1.6.0" do_and_check_cmd ./configure --prefix=/opt/kp-hardened-nginx/deps
CHANGE_DIR="/tmp/kp-hardened-nginx/libmaxminddb-1.6.0" do_and_check_cmd make -j $NTASK
CHANGE_DIR="/tmp/kp-hardened-nginx/libmaxminddb-1.6.0" do_and_check_cmd make install

# Download, compile and install ModSecurity
echo "[*] Clone SpiderLabs/ModSecurity"
# TODO : looks like memory leak is happening with ModSecurity 3.0.5
# so we keep 3.0.4 until a fixed version is available
#git_secure_clone https://github.com/SpiderLabs/ModSecurity.git bf881a4eda343d37629e39ede5e28b70dc4067c0
git_secure_clone https://github.com/SpiderLabs/ModSecurity.git 753145fbd1d6751a6b14fdd700921eb3cc3a1d35
echo "[*] Compile and install ModSecurity"
# temp fix : Debian run it twice
cd /tmp/kp-hardened-nginx/ModSecurity && ./build.sh > /dev/null 2>&1
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" do_and_check_cmd sh build.sh
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" do_and_check_cmd git submodule init
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" do_and_check_cmd git submodule update
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" git_secure_checkout bindings/python 47a6925df187f96e4593afab18dc92d5f22bd4d5
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" git_secure_checkout others/libinjection bf234eb2f385b969c4f803b35fda53cffdd93922
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" git_secure_checkout test/test-cases/secrules-language-tests d03f4c1e930440df46c1faa37d820a919704d9da
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" do_and_check_cmd ./configure --disable-doxygen-doc --disable-dependency-tracking --disable-examples --prefix=/opt/kp-hardened-nginx/deps --with-maxmind=/opt/kp-hardened-nginx/deps
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" do_and_check_cmd make -j $NTASK
CHANGE_DIR="/tmp/kp-hardened-nginx/ModSecurity" do_and_check_cmd make install-strip

# Download and install OWASP Core Rule Set
echo "[*] Clone coreruleset/coreruleset"
git_secure_clone https://github.com/coreruleset/coreruleset.git 18703f1bc47e9c4ec4096853d5fb4e2a204a07a2
echo "[*] Install coreruleset"
do_and_check_cmd mkdir /opt/kp-hardened-nginx/crs
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/coreruleset/rules/* /opt/kp-hardened-nginx/crs
do_and_check_cmd cp /tmp/kp-hardened-nginx/coreruleset/crs-setup.conf.example /opt/kp-hardened-nginx/crs-setup.conf

# Download ModSecurity-nginx module
echo "[*] Clone SpiderLabs/ModSecurity-nginx"
git_secure_clone https://github.com/SpiderLabs/ModSecurity-nginx.git 2497e6ac654d0b117b9534aa735b757c6b11c84f

# Download headers more module
echo "[*] Clone openresty/headers-more-nginx-module"
git_secure_clone https://github.com/openresty/headers-more-nginx-module.git f85af9649b858e21b400a2150a4c7b8ebd36e921

# Download GeoIP module
echo "[*] Clone leev/ngx_http_geoip2_module"
git_secure_clone https://github.com/leev/ngx_http_geoip2_module.git 1cabd8a1f68ea3998f94e9f3504431970f848fbf
do_and_check_cmd sed -i '1s:^:ngx_feature_path=/opt/kp-hardened-nginx/deps/include\n:' /tmp/kp-hardened-nginx/ngx_http_geoip2_module/config
do_and_check_cmd sed -i 's:^ngx_feature_libs=.*$:ngx_feature_libs="-Wl,-rpath,/opt/kp-hardened-nginx/deps/lib -L/opt/kp-hardened-nginx/deps/lib -lmaxminddb":' /tmp/kp-hardened-nginx/ngx_http_geoip2_module/config

# Download cookie flag module
echo "[*] Clone AirisX/nginx_cookie_flag_module"
git_secure_clone https://github.com/AirisX/nginx_cookie_flag_module.git c4ff449318474fbbb4ba5f40cb67ccd54dc595d4

# Download brotli module
echo "[*] Clone google/ngx_brotli"
git_secure_clone https://github.com/google/ngx_brotli.git 9aec15e2aa6feea2113119ba06460af70ab3ea62

# Download lua-nginx module
git_secure_clone https://github.com/openresty/lua-nginx-module.git 9007d673e28938f5dfa7720438991e22b794d225

# Download, compile and install luajit2
echo "[*] Clone openresty/luajit2"
git_secure_clone https://github.com/openresty/luajit2.git 5ff674c5d9b75d6018994dfac3ce38aab3b8db12
echo "[*] Compile luajit2"
CHANGE_DIR="/tmp/kp-hardened-nginx/luajit2" do_and_check_cmd make -j $NTASK
echo "[*] Install luajit2"
CHANGE_DIR="/tmp/kp-hardened-nginx/luajit2" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download and install lua-resty-core
echo "[*] Clone openresty/lua-resty-core"
git_secure_clone https://github.com/openresty/lua-resty-core.git 12f26310a35e45c37157420f7e1f395a0e36e457
echo "[*] Install lua-resty-core"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-core" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download and install lua-resty-lrucache
echo "[*] Clone openresty/lua-resty-lrucache"
git_secure_clone https://github.com/openresty/lua-resty-lrucache.git f20bb8ac9489ba87d90d78f929552c2eab153caa
echo "[*] Install lua-resty-lrucache"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-lrucache" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download and install lua-resty-dns
echo "[*] Clone openresty/lua-resty-dns"
git_secure_clone https://github.com/openresty/lua-resty-dns.git 869d2fbb009b6ada93a5a10cb93acd1cc12bd53f
echo "[*] Install lua-resty-dns"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-dns" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download and install lua-resty-session
echo "[*] Clone bungle/lua-resty-session"
git_secure_clone https://github.com/bungle/lua-resty-session.git 2cd1f8484fdd429505ac33abf7a44adda1f367bf
echo "[*] Install lua-resty-session"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/lua-resty-session/lib/resty/* /opt/kp-hardened-nginx/deps/lib/lua/resty

# Download and install lua-resty-random
echo "[*] Clone bungle/lua-resty-random"
git_secure_clone https://github.com/bungle/lua-resty-random.git 17b604f7f7dd217557ca548fc1a9a0d373386480
echo "[*] Install lua-resty-random"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-random" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download and install lua-resty-string
echo "[*] Clone openresty/lua-resty-string"
git_secure_clone https://github.com/openresty/lua-resty-string.git 3624678ca1c7c32e2fb16c18b7511863e074d542
echo "[*] Install lua-resty-string"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-string" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download, compile and install lua-cjson
echo "[*] Clone openresty/lua-cjson"
git_secure_clone https://github.com/openresty/lua-cjson.git 0df488874f52a881d14b5876babaa780bb6200ee
echo "[*] Compile lua-cjson"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-cjson" do_and_check_cmd make LUA_INCLUDE_DIR=/opt/kp-hardened-nginx/deps/include -j $NTASK
echo "[*] Install lua-cjson"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-cjson" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps LUA_CMODULE_DIR=/opt/kp-hardened-nginx/deps/lib/lua LUA_MODULE_DIR=/opt/kp-hardened-nginx/deps/lib/lua install
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-cjson" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps LUA_CMODULE_DIR=/opt/kp-hardened-nginx/deps/lib/lua LUA_MODULE_DIR=/opt/kp-hardened-nginx/deps/lib/lua install-extra

# Download, compile and install lua-gd
echo "[*] Clone ittner/lua-gd"
git_secure_clone https://github.com/ittner/lua-gd.git 2ce8e478a8591afd71e607506bc8c64b161bbd30
echo "[*] Compile lua-gd"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-gd" do_and_check_cmd make "CFLAGS=-O3 -Wall -fPIC -fomit-frame-pointer -I/opt/kp-hardened-nginx/deps/include -DVERSION=\\\"2.0.33r3\\\"" "LFLAGS=-shared -L/opt/kp-hardened-nginx/deps/lib -llua -lgd -Wl,-rpath=/opt/kp-hardened-nginx/deps/lib" LUABIN=/opt/kp-hardened-nginx/deps/bin/lua -j $NTASK
echo "[*] Install lua-gd"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-gd" do_and_check_cmd make INSTALL_PATH=/opt/kp-hardened-nginx/deps/lib/lua install

# Download and install lua-resty-http
echo "[*] Clone ledgetech/lua-resty-http"
git_secure_clone https://github.com/ledgetech/lua-resty-http.git 9bf951dfe162dd9710a0e1f4525738d4902e9d20
echo "[*] Install lua-resty-http"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-http" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps install

# Download and install lualogging
echo "[*] Clone Neopallium/lualogging"
git_secure_clone https://github.com/lunarmodules/lualogging.git 5973188a1f8fc31abd98aceed2a4853986d779e9
echo "[*] Install lualogging"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/lualogging/src/* /opt/kp-hardened-nginx/deps/lib/lua

# Download, compile and install luasocket
echo "[*] Clone diegonehab/luasocket"
git_secure_clone https://github.com/diegonehab/luasocket.git 5b18e475f38fcf28429b1cc4b17baee3b9793a62
echo "[*] Compile luasocket"
CHANGE_DIR="/tmp/kp-hardened-nginx/luasocket" do_and_check_cmd make LUAINC_linux=/opt/kp-hardened-nginx/deps/include -j $NTASK
echo "[*] Install luasocket"
CHANGE_DIR="/tmp/kp-hardened-nginx/luasocket" do_and_check_cmd make prefix=/opt/kp-hardened-nginx/deps CDIR_linux=lib/lua LDIR_linux=lib/lua install

# Download, compile and install luasec
echo "[*] Clone brunoos/luasec"
git_secure_clone https://github.com/brunoos/luasec.git d5df31561751ec0d4098dfc09c92ece215a56a5a
echo "[*] Compile luasec"
CHANGE_DIR="/tmp/kp-hardened-nginx/luasec" do_and_check_cmd make INC_PATH=-I/opt/kp-hardened-nginx/deps/include linux -j $NTASK
echo "[*] Install luasec"
CHANGE_DIR="/tmp/kp-hardened-nginx/luasec" do_and_check_cmd make LUACPATH=/opt/kp-hardened-nginx/deps/lib/lua LUAPATH=/opt/kp-hardened-nginx/deps/lib/lua install

# Download and install lua-resty-iputils
echo "[*] Clone hamishforbes/lua-resty-iputils"
git_secure_clone https://github.com/hamishforbes/lua-resty-iputils.git 3151d6485e830421266eee5c0f386c32c835dba4
echo "[*] Install lua-resty-iputils"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-iputils" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps LUA_LIB_DIR=/opt/kp-hardened-nginx/deps/lib/lua install

# Download and install lua-resty-redis
echo "[*] Clone openresty/lua-resty-redis"
git_secure_clone https://github.com/openresty/lua-resty-redis.git 91585affcd9a8da65cb664a5b1e926dde428095a
echo "[*] Install lua-resty-redis"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-redis" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps LUA_LIB_DIR=/opt/kp-hardened-nginx/deps/lib/lua install

# Download and install lua-resty-upload
echo "[*] Clone openresty/lua-resty-upload"
git_secure_clone https://github.com/openresty/lua-resty-upload.git 7baca92c7e741979ae5857989bbf6cc0402d6126
echo "[*] Install lua-resty-upload"
CHANGE_DIR="/tmp/kp-hardened-nginx/lua-resty-upload" do_and_check_cmd make PREFIX=/opt/kp-hardened-nginx/deps LUA_LIB_DIR=/opt/kp-hardened-nginx/deps/lib/lua install

# Download nginx and decompress sources
echo "[*] Download nginx-${NGINX_VERSION}.tar.gz"
do_and_check_cmd wget -O "/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}.tar.gz" "https://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz"
do_and_check_cmd wget -O "/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}.tar.gz.asc" "https://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz.asc"
get_sign_source_keys > /tmp/kp-hardened-nginx/nginx.key
do_and_check_cmd gpg --import /tmp/kp-hardened-nginx/nginx.key
do_and_check_cmd gpg --verify /tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}.tar.gz.asc /tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}.tar.gz
CHANGE_DIR="/tmp/kp-hardened-nginx" do_and_check_cmd tar -xvzf nginx-${NGINX_VERSION}.tar.gz

# Compile dynamic modules
echo "[*] Compile dynamic modules"
CONFARGS="$(nginx -V 2>&1 | sed -n -e 's/^.*arguments: //p')"
CONFARGS="${CONFARGS/-Os -fomit-frame-pointer -g/-Os}"
if [ "$OS" = "fedora" ] ; then
	CONFARGS="$(echo -n "$CONFARGS" | sed "s/--with-ld-opt='.*'//" | sed "s/--with-cc-opt='.*'//")"
fi
echo "\#!/bin/bash" > "/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}/configure-fix.sh"
echo "./configure $CONFARGS --add-dynamic-module=/tmp/kp-hardened-nginx/ModSecurity-nginx --add-dynamic-module=/tmp/kp-hardened-nginx/headers-more-nginx-module --add-dynamic-module=/tmp/kp-hardened-nginx/ngx_http_geoip2_module --add-dynamic-module=/tmp/kp-hardened-nginx/nginx_cookie_flag_module --add-dynamic-module=/tmp/kp-hardened-nginx/lua-nginx-module --add-dynamic-module=/tmp/kp-hardened-nginx/ngx_brotli" >> "/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}/configure-fix.sh"
do_and_check_cmd chmod +x "/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}/configure-fix.sh"
CHANGE_DIR="/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}" LUAJIT_LIB="/opt/kp-hardened-nginx/deps/lib -Wl,-rpath,/opt/kp-hardened-nginx/deps/lib" LUAJIT_INC="/opt/kp-hardened-nginx/deps/include/luajit-2.1" MODSECURITY_LIB="/opt/kp-hardened-nginx/deps/lib" MODSECURITY_INC="/opt/kp-hardened-nginx/deps/include" do_and_check_cmd ./configure-fix.sh
CHANGE_DIR="/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}" do_and_check_cmd make -j $NTASK modules
# TODO : move modules to /opt/kp-hardened-nginx/modules
if [ ! -d "/usr/lib/nginx/modules" ] ; then
	do_and_check_cmd mkdir -p /usr/lib/nginx/modules
fi
do_and_check_cmd chown -R root:root /usr/lib/nginx
do_and_check_cmd chmod -R 755 /usr/lib/nginx
CHANGE_DIR="/tmp/kp-hardened-nginx/nginx-${NGINX_VERSION}" do_and_check_cmd cp ./objs/*.so /usr/lib/nginx/modules
do_and_check_cmd chmod 744 /usr/lib/nginx/modules/*

# Remove alpine build dependencies
cd "$old_dir"
cleanup
echo "[*] Dependencies for kp-hardened-nginx successfully installed !"

# Install dependencies
echo "[*] Update packet list"
do_and_check_cmd apt update

echo "[*] Install runtime dependencies"
DEBIAN_DEPS="certbot git cron curl python3 python3-pip procps sudo"
DEBIAN_FRONTEND=noninteractive do_and_check_cmd apt install -y $DEBIAN_DEPS


# Install Python dependencies
echo "[*] Install python dependencies"
do_and_check_cmd pip3 install --upgrade pip
do_and_check_cmd pip3 install -r /tmp/kp-hardened-nginx/gen/requirements.txt
do_and_check_cmd pip3 install -r /tmp/kp-hardened-nginx/jobs/requirements.txt
do_and_check_cmd pip3 install -r /tmp/kp-hardened-nginx/ui/requirements.txt

do_and_check_cmd pip3 install cryptography --upgrade

# Copy generator
echo "[*] Copy generator"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/gen /opt/kp-hardened-nginx

# Copy entrypoint
echo "[*] Copy entrypoint"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/entrypoint /opt/kp-hardened-nginx

# Copy configs
echo "[*] Copy configs"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/confs /opt/kp-hardened-nginx
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/confs /opt/kp-hardened-nginx/confs

# Copy jobs
echo "[*] Copy jobs"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/jobs /opt/kp-hardened-nginx

# Copy LUA
echo "[*] Copy LUA"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/lua /opt/kp-hardened-nginx

# Copy antibot
echo "[*] Copy antibot"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/antibot /opt/kp-hardened-nginx

# Copy defaults
echo "[*] Copy defaults"
do_and_check_cmd cp -r /tmp/kp-hardened-nginx/defaults /opt/kp-hardened-nginx

# Copy settings
echo "[*] Copy settings"
do_and_check_cmd cp /tmp/kp-hardened-nginx/settings.json /opt/kp-hardened-nginx

# Copy sample variables.env
echo "[*] Copy sample variables.env"
do_and_check_cmd cp /tmp/kp-hardened-nginx/misc/variables.env /opt/kp-hardened-nginx

# # Copy UI
# if [ "$OS" != "alpine" ] ; then
# 	echo "[*] Copy UI"
# 	do_and_check_cmd cp -r /tmp/kp-hardened-nginx/ui /opt/kp-hardened-nginx
# 	do_and_check_cmd cp /tmp/kp-hardened-nginx/ui/kp-hardened-nginx-ui.service /lib/systemd/system
# fi

# Copy kp-hardened-nginx
echo "[*] Copy kp-hardened-nginx"
do_and_check_cmd cp /tmp/kp-hardened-nginx/helpers/kp-hardened-nginx /usr/local/bin

# Copy VERSION
echo "[*] Copy VERSION"
do_and_check_cmd cp /tmp/kp-hardened-nginx/VERSION /opt/kp-hardened-nginx

# Replace old nginx.service file
# if [ "$OS" != "alpine" ] ; then
# 	do_and_check_cmd mv /lib/systemd/system/nginx.service /lib/systemd/system/nginx.service.bak
# 	do_and_check_cmd cp /tmp/kp-hardened-nginx/misc/nginx.service /lib/systemd/system/
# fi

# Create nginx user
if [ "$(grep "nginx:" /etc/passwd)" = "" ] ; then
	echo "[*] Add nginx user"
	do_and_check_cmd useradd -d /opt/kp-hardened-nginx -s /usr/sbin/nologin nginx
fi

# Create www folder
if [ ! -d "/opt/kp-hardened-nginx/www" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/www folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/www
fi

# Create http-confs folder
if [ ! -d "/opt/kp-hardened-nginx/http-confs" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/http-confs folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/http-confs
fi

# Create server-confs folder
if [ ! -d "/opt/kp-hardened-nginx/server-confs" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/server-confs folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/server-confs
fi

# Create modsec-confs folder
if [ ! -d "/opt/kp-hardened-nginx/modsec-confs" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/modsec-confs folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/modsec-confs
fi

# Create modsec-crs-confs folder
if [ ! -d "/opt/kp-hardened-nginx/modsec-crs-confs" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/modsec-crs-confs folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/modsec-crs-confs
fi

# Create cache folder
if [ ! -d "/opt/kp-hardened-nginx/cache" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/cache folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/cache
fi

# Create pre-server-confs folder
if [ ! -d "/opt/kp-hardened-nginx/pre-server-confs" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/pre-server-confs folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/pre-server-confs
fi

# Create acme-challenge folder
if [ ! -d "/opt/kp-hardened-nginx/acme-challenge" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/acme-challenge folder"
	do_and_check_cmd mkdir -p /opt/kp-hardened-nginx/acme-challenge/.well-known/acme-challenge
fi

# Create plugins folder
if [ ! -d "/opt/kp-hardened-nginx/plugins" ] ; then
	echo "[*] Create /opt/kp-hardened-nginx/plugins folder"
	do_and_check_cmd mkdir /opt/kp-hardened-nginx/plugins
fi

# Set permissions for /opt/kp-hardened-nginx
echo "[*] Set permissions on files and folders"
do_and_check_cmd chown -R root:nginx /opt/kp-hardened-nginx
do_and_check_cmd find /opt/kp-hardened-nginx -type f -exec chmod 0740 {} \;
do_and_check_cmd find /opt/kp-hardened-nginx -type d -exec chmod 0750 {} \;
do_and_check_cmd chmod 770 /opt/kp-hardened-nginx/cache
do_and_check_cmd chmod -R 770 /opt/kp-hardened-nginx/acme-challenge
do_and_check_cmd chmod 750 /opt/kp-hardened-nginx/entrypoint/*
do_and_check_cmd chmod 750 /opt/kp-hardened-nginx/gen/main.py
do_and_check_cmd chmod 750 /opt/kp-hardened-nginx/jobs/main.py
do_and_check_cmd chmod 750 /opt/kp-hardened-nginx/jobs/reload.py
do_and_check_cmd chmod 750 /opt/kp-hardened-nginx/jobs/certbot-*.py
# Set permissions for /usr/local/bin/kp-hardened-nginx
do_and_check_cmd chown root:root /usr/local/bin/kp-hardened-nginx
do_and_check_cmd chmod 750 /usr/local/bin/kp-hardened-nginx
# Set permissions for /opt
do_and_check_cmd chmod u+rx /opt
# Set permissions for /etc/nginx
do_and_check_cmd chown -R nginx:nginx /etc/nginx
do_and_check_cmd find /etc/nginx -type f -exec chmod 0774 {} \;
do_and_check_cmd find /etc/nginx -type d -exec chmod 0775 {} \;
# Set permissions for systemd files and reload config
do_and_check_cmd chown root:root /lib/systemd/system/kp-hardened-nginx-ui.service
do_and_check_cmd chmod 744 /lib/systemd/system/kp-hardened-nginx-ui.service
do_and_check_cmd chown root:root /lib/systemd/system/nginx.service
do_and_check_cmd chmod 744 /lib/systemd/system/nginx.service
do_and_check_cmd systemctl daemon-reload

# Allow RX access to others on /opt/kp-hardened-nginx
do_and_check_cmd chmod 755 /opt/kp-hardened-nginx
# Allow nginx group to do nginx reload as root
# do_and_check_cmd chown root:nginx /opt/kp-hardened-nginx/ui/linux.sh
# do_and_check_cmd chmod 750 /opt/kp-hardened-nginx/ui/linux.sh
# echo "nginx ALL=(root:root) NOPASSWD: /opt/kp-hardened-nginx/ui/linux.sh" >> /etc/sudoers

# Prepare log files and folders
echo "[*] Prepare log files and folders"
if [ ! -e "/var/log/nginx" ] ; then
	do_and_check_cmd mkdir /var/log/nginx
fi
if [ ! -e "/var/log/nginx/access.log" ] ; then
	do_and_check_cmd touch /var/log/nginx/access.log
fi
if [ ! -e "/var/log/nginx/error.log" ] ; then
	do_and_check_cmd touch /var/log/nginx/error.log
fi
if [ ! -e "/var/log/nginx/modsec_audit.log" ] ; then
	do_and_check_cmd touch /var/log/nginx/modsec_audit.log
fi
if [ ! -e "/var/log/nginx/jobs.log" ] ; then
	do_and_check_cmd touch /var/log/nginx/jobs.log
fi
if [ ! -e "/var/log/nginx/ui.log" ] ; then
	do_and_check_cmd touch /var/log/nginx/ui.log
fi
do_and_check_cmd chown -R root:nginx /var/log/nginx
do_and_check_cmd chmod -R 770 /var/log/nginx/

# Install cron
echo "[*] Add jobs to crontab"
CRON_SRC="/tmp/kp-hardened-nginx/misc/cron-linux"
CRON_PATH="/etc/cron.d/kp-hardened-nginx"
do_and_check_cmd cp "$CRON_SRC" "$CRON_PATH"
do_and_check_cmd chown root:nginx "$CRON_PATH"
do_and_check_cmd chmod 740 "$CRON_PATH"

# Download abusers list
echo "[*] Download abusers list"
do_and_check_cmd su -s "/bin/bash" -c "/opt/kp-hardened-nginx/jobs/main.py --name abusers" nginx

# Download TOR exit nodes list
echo "[*] Download TOR exit nodes list"
do_and_check_cmd su -s "/bin/bash" -c "/opt/kp-hardened-nginx/jobs/main.py --name exit-nodes" nginx

# Download proxies list
echo "[*] Download proxies list"
do_and_check_cmd su -s "/bin/bash" -c "/opt/kp-hardened-nginx/jobs/main.py --name proxies" nginx

# Download referrers list
echo "[*] Download referrers list"
do_and_check_cmd su -s "/bin/bash" -c "/opt/kp-hardened-nginx/jobs/main.py --name referrers" nginx

# Download user agents list
echo "[*] Download user agents list"
do_and_check_cmd su -s "/bin/bash" -c "/opt/kp-hardened-nginx/jobs/main.py --name user-agents" nginx

# Download geoip database
echo "[*] Download geoip DB"
do_and_check_cmd su -s "/bin/bash" -c "/opt/kp-hardened-nginx/jobs/main.py --name geoip" nginx

# We're done
cd "$old_dir"
cleanup
echo "[*] kp-hardened-nginx successfully installed !"
